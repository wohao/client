...
def imagemodify():
    ...
    for root, dirs, files in os.walk(images_path):
        ...
        if files:
            ...
            filedirs = "./133h333"+str(checkNo) #新建文件路径
            #print(filedirs)
            os.mkdir(filedirs)
            i =0
...
Entry(root,textvariable=select_path).grid(row=1,column=1)
...
Entry(root,textvariable=save_path).grid(row=2,column=1)
...
Entry(root).grid(row=3,column=1,sticky=E)
...
Button(root,text="执行",command=imagemodify).grid(column=1,sticky=N)
